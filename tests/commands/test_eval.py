...
def test_success_eval_simple_model():
    ...
    save_path_eval = "tmp-eval-save-path/config.json"
...
"""
    ...
    command_eval = application.find("eval")
    eval_tester = CommandTester(command_eval)
    eval_tester.execute(
        save_path
        + " "
        + dataset_module
    ...
    os.remove(module_file_path)
    os.remove(dataset_module_file_path)
    os.remove(save_path_eval)
    shutil.rmtree(os.path.dirname(save_path_eval))
    shutil.rmtree(os.path.dirname(save_path))
